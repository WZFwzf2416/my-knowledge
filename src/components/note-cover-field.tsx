  ...
  return (
    <div className="rounded-[1.75rem] border border-border bg-surface/70 p-5">
      <div className="flex flex-col gap-4">
        ...
        <div
          className="h-44 rounded-[1.5rem] border border-border bg-background"
          style={previewStyle}
          aria-label="封面图预览"
        >
          {previewUrl || coverImageUrl ? null : (
            <div className="flex h-full items-center justify-center text-sm text-muted">还没有封面图</div>
          )}
        </div>
      ...
      <div className="mt-5 rounded-2xl border border-dashed border-border bg-background/70 p-4">
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-xs leading-5 text-muted">如果你已经有在线图片地址，也可以直接粘贴链接作为封面图。</p>
          </div>
          <button
            type="button"
