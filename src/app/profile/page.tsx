...
export default async function ProfilePage({
  ...
  const fallbackLabel =
    profile.nickname?.trim()?.[0]?.toUpperCase() ?? profile.email[0]?.toUpperCase() ?? "M";
    ...
    <main className="bg-background min-h-screen px-6 py-12 sm:px-10 lg:px-12">
      <div className="page-enter mx-auto max-w-6xl space-y-6">
        <header className="glass-card rounded-[2rem] p-8 sm:p-10">
          <p className="text-accent text-sm font-medium tracking-[0.3em] uppercase">个人资料</p>
        ...
        {message ? (
          <div className="fade-in border-accent/20 bg-accent/10 text-foreground rounded-[2rem] border px-6 py-4 text-sm leading-6">
            {message}
          </div>
        ) : null}
        ...
        <section className="grid gap-6 lg:grid-cols-[1fr_320px]">
            ...
            <div className="grid gap-5">
              <label className="text-muted block text-sm">
                ...
                <input
                  ...
                  value={profile.email}
                  disabled
                  className="border-border bg-surface text-foreground mt-2 w-full rounded-2xl border px-4 py-3 opacity-70"
                />
              </label>
          ...
          <aside className="space-y-6">
            <div className="soft-card rounded-[2rem] p-6">
              <h2 className="text-foreground text-xl font-semibold">当前资料</h2>
              <div className="mt-5 flex items-center gap-4">
                <div
                  className="bg-accent flex h-16 w-16 items-center justify-center rounded-full text-xl font-semibold !text-white"
                  style={
                    profile.avatarUrl
                      ? {
              ...
              <p className="text-muted mt-4 text-sm leading-7">
                {profile.bio ||
                  "还没有填写个人简介。你可以把这里当作一块很小的个人说明，也可以记录当前项目的目标和状态。"}
              </p>
              {profile.avatarUrl ? (
                <p className="text-muted mt-4 text-xs leading-6 break-all">
            ...
            <div className="soft-card rounded-[2rem] p-6">
              <h2 className="text-foreground text-xl font-semibold">当前统计</h2>
            ...
            <div className="soft-card text-muted rounded-[2rem] p-6 text-sm leading-6">
              <p className="text-foreground font-medium">Storage 提示</p>
              <p className="mt-3">
                默认 bucket 名称是 `avatars`。如果保存头像失败，请先到 Supabase Storage 创建同名
                bucket，并为已登录用户配置上传权限。
              </p>
            </div>
          </aside>
        </section>
